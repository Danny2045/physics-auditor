...
def main():
    ...
    claims = {
        ...
        "pending_followup": (
            "The 'structurally pending' status of this finding is now "
            "RESOLVED on two of three originally-named confirmation "
            "steps. (1) Kabsch convergent validation — DONE: the v2 "
            "engine in src/physics_auditor/causality/superposition.py "
            "reproduces all three cluster pairings (HIS185<->HIS56, "
            "PHE188<->ALA59, ARG265<->ARG136) under both the FMN "
            "anchor (31 atoms, anchor RMSD 0.26 Å) and the ORO "
            "orthogonal cross-check anchor (11 atoms, anchor RMSD "
            "0.08 Å). See PfDHODH_vs_HsDHODH_kabsch_convergence.json "
            "for per-residue distances and runner-up margins. "
            "(2) Published structural-biology cross-check — DONE: see "
            "LITERATURE.md in this directory. The HIS185<->HIS56 and "
            "ARG265<->ARG136 pairings are tier-C3 (peer-reviewed "
            "primary literature including the 1998 His56/H185 "
            "mutagenesis study, ScienceDirect S0006295297001974); the "
            "PHE188<->ALA59 substitution is C2-strong (corroborated "
            "independently on each side via PMC2883174 and recent "
            "ACS Omega/BMC binding-site analyses) with the structural "
            "margin note that PHE188 is FMN-anchor-clean but "
            "ORO-anchor-cutoff-sensitive — biologically coherent for "
            "the one SUBSTITUTED cluster residue. (3) TM-align "
            "gold-standard structural alignment — outstanding; "
            "remains the cleanest residual independent check. With "
            "(1) and (2) complete, this finding is "
            "'sequence-confirmed AND structurally-confirmed by "
            "in-repo convergent Kabsch + C3 literature; TM-align "
            "remains pending as the residual gold-standard gate.' "
            "An additional methodological lead is in LITERATURE.md: "
            "PDB 4OQV is HsDHODH bound to DSM338 (same compound as "
            "4ORM), so a 4ORM-vs-4OQV rerun would remove the "
            "chemotype confound present in this 4ORM-vs-1D3H pair."
        ),
    }
