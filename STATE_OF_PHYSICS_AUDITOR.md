...
- **Do not introduce new scientific claims or new metrics in this document.** If a number is not already documented in committed state (a result artifact, a findings doc), it does not belong here. This file is a faithful summary; it is not a place to publish.
...
The 14-pair binding-site clashscore benchmark manuscript (formerly `manuscript/preprint.md` + `preprint.pdf`) was withdrawn from master in PR `chore/withdraw-manuscript`. It described a benchmark that has since been corrected (PR-A1 withdrew three invalid pairs; PR-DET corrected stale clashscore numbers). The canonical record of benchmark results is now `benchmark/results/pocket_clashscore_comparison.json` plus the per-finding documentation in this state document. A new manuscript will be written from scratch once the benchmark substitutions (PR-B/C/D) and the broader system are in place. Git history preserves the v0.1 draft (last at commit `96eac03`).
...
- **Mislabel issue (historical):** Pair 2 was labeled `SmDHODH` but used `1D3H.pdb` whose SOURCE record reads `HOMO SAPIENS`. The structure is the catalytic construct of HsDHODH (UniProt Q02127), not the parasite enzyme. The AlphaFold side (`AF-G4VFD7-F1-model_v6.pdb`) was the true SmDHODH; the experimental side was mislabeled. The mislabel propagated into the (now-withdrawn) manuscript Table 1. See Finding 1.

### 7.3 14-pair benchmark manuscript (WITHDRAWN)

This manuscript was withdrawn from master in PR `chore/withdraw-manuscript`. It reported the 14-pair binding-site clashscore benchmark, but that benchmark contained two wrong-protein rows (Findings 8, 9), one mislabel (Finding 1), and stale clashscore numbers (Finding 11), all since corrected. Rather than patch the manuscript pair-by-pair, it was removed; a new manuscript will be written against the corrected benchmark once the substitution PRs (PR-B/C/D) restore the withdrawn rows. Git history preserves the v0.1 draft (last at commit `96eac03`). The canonical results record is `benchmark/results/pocket_clashscore_comparison.json` plus this document's findings.
...
**Status:** **Closed.** PR `fix/benchmark-mislabel-pair1-and-causality-artifacts` (PR-A1) corrected the BENCHMARK_PAIRS schema: pair 1 mapped to 1D3H ↔ AF-Q02127 (true HsDHODH-vs-HsDHODH); pairs 2, 3, 5 removed pending substitutions (PR-B, PR-D, PR-C); pair 8 annotated for AKAP9-BRAF construct. Causality artifact filenames and internal labels corrected. The `benchmark/results/pocket_clashscore_comparison.json` regeneration shipped in PR-DET (Finding 11 closure). The manuscript that propagated this mislabel has been withdrawn from master (PR `chore/withdraw-manuscript`); there is no manuscript Table 1 to update. Finding 1 is now fully closed: the benchmark JSON is corrected, the causality artifacts are relabeled, and the manuscript that carried the mislabel is gone.

`benchmark/results/pocket_clashscore_comparison.json` (pair 2) labeled `1D3H.pdb` as "SmDHODH" and paired it with the AlphaFold model `AF-G4VFD7-F1-model_v6.pdb` (which *is* SmDHODH). But `1D3H.pdb`'s SOURCE record reads `ORGANISM_SCIENTIFIC: HOMO SAPIENS`; the structure is the catalytic construct of *human* DHODH (UniProt Q02127), not the parasite. The numerical clashscore values were valid for the underlying experimental structure (1D3H); the *label* "SmDHODH" attached to it was wrong.

The mislabel propagated into the (now withdrawn) `manuscript/preprint.md` Table 1, where the "SmDHODH" row read `1D3H | G4VFD7 | …` — pairing a human PDB with a parasite UniProt ID. The same conflation was in the producing script's `BENCHMARK_PAIRS` description string: *"Parasite DHODH — 30.8x selectivity target."*

The mislabel was originally caught and recorded during the `feat/divergence` review on 2026-05-11 (commit `a19a2a2`: *"relabel 1D3H/1MVS run — HsDHODH vs HsDHFR, not Sm/Hs"*) for a *different* result artifact (the early HsDHODH-vs-HsDHFR selectivity-map run, now `benchmark/results/selectivity_maps/HsDHODH_vs_HsDHFR_pocket_attribution.json`). The 14-pair benchmark and the manuscript were not updated at that time; PR #1 added the provenance machinery and used it correctly for the PfDHODH run, but did not retro-fix the master-side artifacts. PR-A1 and PR-DET corrected the benchmark side; PR `chore/withdraw-manuscript` resolved the manuscript side by withdrawing it.

**Resolution:** Finding 1 is closed. The benchmark-side corrections shipped in PR-A1 and PR-DET; the manuscript that carried the mislabel was withdrawn rather than patched. No further PR is required for this finding.
...
PR-A1 annotates the `BENCHMARK_PAIRS` description for pair 8 to explicitly declare the AKAP9-BRAF fusion construct. The AKAP9-BRAF construct is documented in the BENCHMARK_PAIRS description (PR-A1) and in this finding. The manuscript that would have footnoted it has been withdrawn (PR `chore/withdraw-manuscript`); a future manuscript will document the construct when written. No substitution is planned because the atomic data is correct BRAF kinase domain.
...
**Downstream:** the manuscript that carried the stale clashscore numbers has been withdrawn from master (PR `chore/withdraw-manuscript`) rather than updated. The corrected numbers live in `benchmark/results/pocket_clashscore_comparison.json`, locked by the regression test added in PR-DET.

### Status summary

| Finding | Status | Origin |
|---|---|---|
| 1. Mislabel propagation (1D3H labeled "SmDHODH" in master benchmark + manuscript) | Closed (benchmark corrected in PR-A1/PR-DET; manuscript withdrawn in `chore/withdraw-manuscript`) | **Surfaced by this audit** |
...
4. **Boltz-2 and Chai-1 placeholder directories.** `benchmark/structures/predicted/{boltz2,chai1}/` are empty. Extending the benchmark to additional predictors (Boltz-2, Chai-1) is a natural next step (this was noted in the now-withdrawn manuscript's limitations section; it remains a valid direction). No code references them. Should the directories stay (as a visible roadmap signal) or be removed (until there is something to commit)?
...
6. **DEFERRED (deliberate, external-data dependency): true-ortholog pocket-divergence number.** The divergence runner produces `full_sequence_cosine = 0.989732` for true SmDHODH vs HsDHODH UniProt sequences (the sequence leg works and is committed). The **pocket** leg is unmeasured, and is intentionally left unmeasured: computing it requires a *parasite-side DHODH co-crystal with a bound ligand* to anchor pocket-residue indices via the `compute_selectivity_map` convention, and no such structure is committed (the parasite side is the apo AlphaFold model `AF-G4VFD7`, which has no ligand). This is an **external-data acquisition decision, not an engine task** — the engine is ready; the input does not exist in-repo. Closing it would mean fetching and provenance-verifying a SmDHODH co-crystal (candidate: `6FMD`, SmDHODH + inhibitor) and committing it to `benchmark/structures/experimental/`, then extending the divergence runner's pocket leg. The Step 3 consolidation (2026-05-29) deliberately did **not** do this — it sharpened this deferral to be explicit rather than reconstructing the path. Note: the other Schisto artifact, the mixed-provenance `HsDHODH_causality.json`, was retired in the same step (see `benchmark/results/causality/archive/README.md`); the two are independent.
7. **RESOLVED: 14-pair benchmark relabel scope.** The benchmark JSON was corrected (PR-A1 withdrew three invalid pairs; PR-DET regenerated the clashscore numbers and locked them under a regression test) and the manuscript was withdrawn (PR `chore/withdraw-manuscript`); this question is closed.
